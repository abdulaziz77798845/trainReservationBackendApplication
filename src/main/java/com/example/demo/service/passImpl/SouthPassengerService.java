...
@Service
public class SouthPassengerService  implements PassengerService{

    @Qualifier("southTemplate")
    @Autowired
    private MongoTemplate southPassTemplate;

    public Passenger addPassenger(Passenger passenger) {
        try {
            southPassTemplate.insert(passenger);
            return passenger;
        }
    ...
    public Optional<Passenger> findPassengerById(String passID){
        try {
            return Optional.ofNullable(southPassTemplate.findById(passID, Passenger.class));
        }
        catch (DataAccessException exp){
    ...
    public void deletePassengerByID(String passID){
        try {
            ...
            southPassTemplate.remove(query, Passenger.class);
        }
        catch (DataAccessException exp){
    ...
    public List<Passenger> getAllPassengersInTheRegion(){
        try {
            return southPassTemplate.findAll(Passenger.class);
        }
        catch (DataAccessException exp){
    ...
    public void savePassenger(Passenger passenger){
        try {
            southPassTemplate.save(passenger);
        }
        catch (DataAccessException exp){
